        ...
        <section className={styles.heroPanel}>
          <div className={styles.heroCopy}>
            ...
            <p className={styles.heroText}>
              Este prototipo reorganiza los datos abiertos de la comunidad en una
              experiencia mas clara y util para consultar actividades culturales,
              localizar descuentos y explorar opciones de ocio con una navegacion
              sencilla, especialmente en movil.
            </p>
        ...
        <section className={styles.contentGrid}>
          <div className={styles.mainColumn}>
            ...
            <section className={`${styles.sectionCard} ${styles.warmCard}`}>
              ...
              <div className={styles.discountGrid}>
                {descuentosDestacados.map((descuento) => (
          ...
          <aside className={styles.sideColumn}>
            <section className={`${styles.sectionCard} ${styles.compactCard}`}>
              ...
              <h3>Fuentes con mayor volumen de contenido</h3>
              <div className={styles.chipCloud}>
                {categoriasEvento.map((item) => (
                  <span key={`evento-${item.categoria}`} className={styles.cloudChip}>
                    {EVENT_LABELS[item.categoria] ?? item.categoria} · {item._count.categoria}
                  </span>
                ))}
                {categoriasDescuento.map((item) => (
                  <span
                    key={`descuento-${item.categoria}`}
                    className={`${styles.cloudChip} ${styles.alt}`}
                  >
                    {DISCOUNT_LABELS[item.categoria] ?? item.categoria} · {item._count.categoria}
                  </span>
                ))}
              </div>
            </section>
